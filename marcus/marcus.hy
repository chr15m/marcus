...
(defn load-bookmarks []
  ...
  (let [places-sqlite (glob (path.expanduser (path.join "~" ".mozilla" "firefox" "*.default" "places.sqlite")))
        db (try (sqlite3.connect (get places-sqlite 0)) (except [e Exception] (sys.exit "Could not find or open the places.sqlite file.")))
        c (db.cursor)]
    (list-comp
...
(defn main [argv]
  (try
    (cond
      ...
      [(> (len sys.argv) 1) (perform-search (cut sys.argv 1))]
      [True (usage sys.argv)])
